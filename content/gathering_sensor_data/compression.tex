...
\\\\
The test was conducted by generating three pseudo random floating point numbers (from $0$ to $1$). These three numbers would then be compressed by the mechanism described previously. This was repeated for a specific amount of compressions. All of the tests were evaluated $1,000$ times, to avoid measuring different noise factors such as CPU starvation. The tests are performed on two different devices: a Nexus 5 and a OnePlus One. Ideally the test should be performed on several different devices on different tiers, however, we do not have access to a big variety of devices, and have thus only tested on two. The results of the tests can be seen in \figref{fig:result_of_compression_test}.
