...
Participants may browse the publicly available campaigns if they want to contribute to a campaign. This can be done through the screen seen in \figref{fig:public_campaigns}. All the publicly available campaigns are listed here, each with its own entry which displays the title alongside the author of the campaign. If the participant wishes to learn more about one of the listed campaigns, he/she can press on that campaign and be redirected to a view similar to the one seen in \figref{fig:campaign_specification}, where more detailed information about the selected campaign is shown. Only the data which is strictly necessary for displaying the list is downloaded initially. The detailed information is only downloaded on demand to use less resources.
\\\\
