...
Some campaigns might request that the participants answer a questionnaire. The request can happen when the participant has closed the application, or when the participant is not using the phone. To accompany this, we chose to use notifications to inform the participant when they should answer questionnaires, which can be seen in \figref{fig:answering_questionnaire_notification}. When the notification is pressed, the application opens the view separate from the rest of the application as seen in \figref{fig:answering_questionnaire_answering}. 
