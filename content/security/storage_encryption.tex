...
The problem was that we used more than the maximum storage size of 0.42 MB as stated in the acceptance test, which roughly corresponds to 10MB in 24 hours.  The JSON format had too much overhead, because it utilizes explicit field names instead of a schema, and thus it used more storage space than what should be needed. 
\\\\
